...
export class CommunityPageMemberDetail {
  ...
  private getStatusHeaderDetail(divIndex: number, spanIndex: number): Locator {
    return this.page
      .locator(this.statusBaseSelector)
      .nth(divIndex)
  ...
  private async getStatusText(
    ...
    console.log(
      `Waiting for status span at divIndex=${divIndex}, spanIndex=${spanIndex} to be visible...`
    );
    await locator.waitFor({ state: "visible", timeout: 5000 });
    const text = await locator.evaluate((el) => el.textContent?.trim() || "");
    console.log(
      `Status text at divIndex=${divIndex}, spanIndex=${spanIndex}: "${text}"`
    );
    return text;
  }
  ...
  private getStatusBadge(divIndex: number): Locator {
    return this.page.locator(
      `${this.statusBaseSelector}:nth-child(${
        divIndex + 1
      }) [data-slot="badge"]`
    );
  }
