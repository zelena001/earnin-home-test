...
export class CommunityPage {
  ...
  get countElement(): Locator {
    return this.page
      .locator('[class="space-y-4"] div:nth-child(1) p:nth-child(1)')
      .last();
  }
  ...
  get tableRows(): Locator {
    return this.isMobile
      ? this.page.locator(
          '[data-testid="members-table-mobile"] > div[data-slot="card"]'
        )
  ...
  private async waitForTable(): Promise<void> {
    ...
    if (this.isMobile) {
      await this.page
        .locator('[data-testid="members-table-mobile"] > div[data-slot="card"]')
        .first()
  ...
  private async selectDate(targetDate: string, useNext = false): Promise<void> {
    ...
    const dateLocator = this.page.locator(
      `[data-testid="date-calendar"] div:nth-child(2) [data-day="${targetDate}"]`
    );
    ...
    while (!(await dateLocator.isVisible())) {
      if (tries++ > maxTries) {
        throw new Error(
          `❌ Could not find target date ${targetDate} after ${maxTries} attempts`
        );
      }
      console.log(
        `Date not visible yet, clicking month button (attempt ${tries})...`
      );
  ...
  async getMemberCountByDateRange(
    fromDate: string,
    toDate: string
  ): Promise<number> {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (
        texts.some((text) => {
          ...
          return (
            rowDate.isValid() &&
            rowDate.isSameOrAfter(from) &&
            rowDate.isSameOrBefore(to)
          );
        })
      ) {
        count++;
